...
public class <%= entityName %> {
    ...
    %><% if (utils.isEntityAttributeRequired(attribute)) { %>
    @NotNull<% } %>
    @Column(name = "<%= utils.entityAttributeColumnOf(attribute) %>")
    ...
    public <%= utils.javaTypeOf(attribute.rawType) %> <%= utils.entityGetterNameOf(attribute) %>() {
        ...
        %>if (<%= utils.entityAttributeNameOf(attribute) %> != null)
            return <%= utils.entityAttributeNameOf(attribute) %>.toInstant();
        else
            return null;<% } else {
    ...
    public <%= entityName %> <%= utils.entitySetterNameOf(attribute) %>(final <%= utils.javaTypeOf(attribute.rawType) %> <%= utils.entityAttributeNameOf(attribute) %>) {
        <% if (utils.javaTypeOf(attribute.rawType) == "Instant") {
        %>if (<%= utils.entityAttributeNameOf(attribute) %> != null)
            this.<%= utils.entityAttributeNameOf(attribute) %> = Timestamp.from(<%= utils.entityAttributeNameOf(attribute) %>);
        else
            this.<%= utils.entityAttributeNameOf(attribute) %> = null;<% } else {
